    ...
    try:
        ...
        if data:
            ...
            for _, row in curr.iterrows():
                name, net = str(row['name']), round((row['buy'] - row['sell']) / 1000)
...
report = [f"🚀 <b>【台股 AI 投資導航】</b>", f"📅 執行時間: {get_taiwan_time().strftime('%m/%d %H:%M')}", "═" * 18]
    ...
    try:
        ...
        if stype == "權值":
            if f_buy > 200 and m_up:
                cmd, icon = "🟢 <b>建議跟單：外資轉多</b>", "✅"
            elif f_buy < -500:
                cmd, icon = "🔴 <b>建議保守：外資提款</b>", "❌"
            else:
                cmd, icon = "🟡 <b>維持觀望：動能不明</b>", "⏸️"
        else: # 小型
            if i_buy > 0 and m_up:
                cmd, icon = "🟢 <b>建議跟單：投信認養</b>", "✨"
            elif i_buy < 0:
                cmd, icon = "🔴 <b>建議保守：投信棄養</b>", "❌"
            else:
                cmd, icon = "🟡 <b>維持觀望：靜待點火</b>", "⏸️"
